...
Builds on processing in *~GitHub/inattention-population/code/inattention-populationsample-data-prep.Rmd*
...
#### Using the 'cvTools' package
http://stackoverflow.com/questions/16717970/cross-validating-a-cart-model
...
# Estimate the prediction error of a model via (repeated) K-fold cross-validation. 
# It is thereby possible to supply an object returned by a model fitting function, 
# a model fitting function itself, or an unevaluated function call to a model fitting function.

cvFit(rpart, formula=averBinned ~ ., data=C,
             cost=function(y, yHat) (y != yHat) + 0,  K = 5, R = 10, predictArgs=c(type='class'))
      
#            cost=function(y, yHat) (y != yHat) + 0, predictArgs=c(type='class'))
```

#### A Multi-Class Summary Function
https://github.com/rseiter/PracticalMLProject/blob/master/multiClassSummary.R
https://github.com/topepo/caret/issues/107
topepo added a commit that referenced this issue on Sep 8, 2015:
...
CART:  method = 'rpart' or Ordinal Responses:  method = 'rpartScore'
with a tuning parameter cp (Complexity Parameter).
...
# number controls with the number of folds in K-fold cross-validation ("cv")
# tc <- trainControl(method = "cv", number = 10, classProbs = TRUE, summaryFunction = multiClassSummary)
tc <- trainControl(method = "cv", number = 10, savePredictions = TRUE)
rpart.grid <- expand.grid(.cp = c(0.2, 0.4))

# train.rpart <- train(averBinned ~., data = C, method = "rpart", trControl = tc, tuneGrid = rpart.grid, metric = "ROC")

train.rpart <- train(averBinned ~., data = C, method = "rpart", trControl = tc, tuneGrid = rpart.grid)
print(train.rpart)
```

```{r, echo=TRUE, eval=TRUE}
# plot(train.rpart)  
ggplot(train.rpart) + theme_bw()
```
...
ps = makeParamSet(
  makeDiscreteParam("cp", values = seq(0,0.4,0.1)),
