...
set.seed(1)
CC <- C
CC$averBinnedC = as.factor(CC$averBinned)  # To perform clasification and not regression
CC <- subset(CC, select = -c(averBinned))
frmla = averBinnedC ~ gender + grade + snap1 + snap2 + snap3 + 
    snap4 + snap5 + snap6 + snap7 + snap8 + snap9
fit.Crf = randomForest(frmla, data=CC, importance = TRUE)
print(fit.Crf)
# plot(fit.Crf)
print(round(importance(fit.Crf), 2))
```
...
```{r, echo=TRUE, eval=TRUE}
round(importance(fit.Crf), 2)
#plot(importance(fit.Crf), lty=2, pch=16)
#lines(importance(fit.Crf))
```
```{r, echo=TRUE, eval=TRUE}
# Dotchart of variable importance as measured by a Random Forest
# varImpPlot(x, sort=TRUE, n.var=min(30, nrow(x$importance)), type=NULL, class=NULL, 
# scale=TRUE, main=deparse(substitute(x)), ...)
varImpPlot(fit.Crf, class = TRUE, main="RF - variable importance")
```
```{r, echo=TRUE, eval=TRUE}
# Find out which predictor variables are actually used in the random forest.
# If count=TRUE and by.tree=FALSE, a integer vector containing frequencies that 
# variables are used in the forest.
frmla
varUsed(fit.Crf, by.tree=FALSE, count=TRUE)
# Size of trees (number of nodes) in and ensemble.
# i.e. a vector containing number of nodes for the trees in the randomForest object.
hist(treesize(fit.Crf, terminal=FALSE))
```
...
rfD.task = classif.task = makeClassifTask(id = "inattentionD.rf", data = D, target = "averBinned")
print(rfD.task)
str(getTaskData(rfD.task))
```

```{r, echo=TRUE, eval=FALSE}
# library(FSelector)
# https://www.i2m.univ-amu.fr/support/doku.php?id=rweka
...
# Let’s see which variables are important. Later, we can use this knowledge to subset out 
# input predictors for model improvement. 
...
# imp_feat <- generateFilterValuesData(rfD.task, method = c("information.gain","chi.squared"))
imp_feat <- generateFilterValuesData(rfD.task,  method = "rf.importance")  # "rf.importance")
plotFilterValues(imp_feat,n.show = 20)
```
...
lrn_prob = makeLearner("classif.randomForest", predict.type = "prob")
mod_prob = train(lrn_prob, rfD.task)
pred_prob = predict(mod_prob, newdata = D)
...
fv = generateFilterValuesData(rfD.task, method = "rf.importance")
print(fv)
print(fv$data)
...
rf_param <- makeParamSet(
  ...
  makeIntegerParam("mtry", lower = 9, upper = 10),
  makeIntegerParam("nodesize", lower = 50, upper = 50)
)
print(rf_param)
```
```{r, echo=TRUE, eval=TRUE}
# Perform random search for 50 iterations
...
# of parameters provided. In a random search, we specify the number of iterations 
# and it randomly passes over the parameter combinations. 
# In this process, it might miss out some important combination of parameters 
# which could have returned maximum accuracy, who knows.
...
res = tuneParams(lrn, task = rfD.task, resampling = resamp, measures = list(acc,timetrain),
                 par.set = rf_param, control = control.grid, show.info = TRUE)
...
names(res)
print(c(res$x$ntree, res$x$mtry, res$x$nodesize))
print(res$y)
```
